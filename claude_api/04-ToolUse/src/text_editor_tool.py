...
class TextEditorTool:
    ...
    def __call__(self, command: str, path: str, **kwargs):
        # The Anthropic text editor tool sends a single tool call with a
        # `command` selector and a `path`; dispatch to the matching method.
        if command == "view":
            return self.view(path, kwargs.get("view_range"))
        elif command == "str_replace":
            return self.str_replace(path, kwargs["old_str"], kwargs["new_str"])
        elif command == "create":
            return self.create(path, kwargs["file_text"])
        elif command == "insert":
            return self.insert(path, kwargs["insert_line"], kwargs["new_str"])
